defmodule Juvet.Template.Compiler.Slack.Elements.PlainTextInput do
  @moduledoc false

  alias Juvet.Template.Compiler.Slack.Objects.{DispatchActionConfig, Text}
  import Juvet.Template.Compiler.Encoder.Helpers, only: [maybe_put: 3]

  def compile(%{element: :plain_text_input, attributes: attrs} = el) do
    %{type: "plain_text_input"}
    |> maybe_put(:action_id, attrs[:action_id])
    |> maybe_put(:initial_value, attrs[:initial_value])
    |> maybe_put(:multiline, attrs[:multiline])
    |> maybe_put(:min_length, attrs[:min_length])
    |> maybe_put(:max_length, attrs[:max_length])
